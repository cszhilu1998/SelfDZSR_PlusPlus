...
# SelfTZSR++ Model
class TZSRModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        return parser

    def __init__(self, opt):
        super(TZSRModel, self).__init__(opt)
    ...
    def set_input(self, input):
        ...
        self.data_ref2x = input['ref2x'].to(self.device)
        ...
        self.coord_ref2x = input['coord_ref2x'].to(self.device)
        self.coord_ref4x = input['coord_ref4x'].to(self.device)
        self.image_name = input['fname']

    def forward(self):
        if not self.opt.isTrain:
            self.stu_out_ref2x = self.netStudent(self.data_lr, self.data_ref2x)
            self.stu_out_ref4x = self.netStudent(self.data_lr, self.data_ref4x)

            self.data_sr = self.netRefSR(
                self.data_lr, self.data_lr, self.stu_out_ref2x, self.stu_out_ref4x, self.data_ref2x, 
                self.data_ref4x, self.data_ref2x_lr, self.coord_ref2x, self.coord_ref4x)		
        else:
            # Using patch-based optical flow
            if self.opt.camera == 'nikon':
                down_hr = F.interpolate(input=self.data_hr, scale_factor=1/self.scale, mode='bilinear', align_corners=True)
                ...
                self.stu_out_ref2x = self.netStudent(self.lr_warp, self.data_ref2x)
                self.stu_out_ref4x = self.netStudent(self.lr_warp, self.data_ref4x)

                self.data_sr = self.netRefSR(
                    self.lr_warp, self.data_down_hr_de, self.stu_out_ref2x, self.stu_out_ref4x, self.data_ref2x, 
                    self.data_ref4x, self.data_ref2x_lr, self.coord_ref2x, self.coord_ref4x)
            ...
            # The full image can be regarded as a small patch during pre-alignment.
                ...
                self.data_sr = self.netRefSR(
                    self.data_lr, self.data_down_hr_de, self.stu_out_ref2x, self.stu_out_ref4x, self.data_ref2x, 
                    self.data_ref4x, self.data_ref2x_lr, self.coord_ref2x, self.coord_ref4x)
...
class SelfRefSR(nn.Module):
    def __init__(self, opt):
        ...
        ref_extractor_1 = [N.MeanShift(),
                           N.conv(3, 64, mode='CR'),
                           N.conv(64, 64, mode='CRCRCRC')]
        self.ref_extractor_1 = N.seq(ref_extractor_1)
        ...
        if self.paste:
            ref_head_1 = [N.MeanShift(),
                          N.DownBlock(2),
                          N.conv(3*2**2, n_feats, mode='C')]
            self.ref_head_1 = N.seq(ref_head_1)
        ...
        self.deform_conv_1 = ops.DeformConv2d(64, 64, kernel_size=3, stride=1, \
        ...
        self.concat_fea_1 = nn.Sequential(
            nn.Conv2d(64*2, 64, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True))
        self.concat_fea_2 = nn.Sequential(
            nn.Conv2d(64*2, 64, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True))
    ...
    def forward(self, lr, hr_bic, stu_out_ref2x, stu_out_ref4x, data_ref2x, data_ref4x, data_ref2x_lr, coord_ref2x, coord_ref4x, paste=True):
        N, C, H, W = lr.size()
        pre_offset_ref2x = self.corr(stu_out_ref2x)
        pre_offset_ref2x = F.interpolate(pre_offset_ref2x, data_ref2x.shape[-2:], mode='bilinear', align_corners=True) * 4
        ref2x_feats = self.ref_extractor_1(data_ref2x)
        ref2x_deform = self.deform_conv_1(ref2x_feats, pre_offset_ref2x)  
        ref2x_deform = F.interpolate(ref2x_deform, lr.shape[-2:], mode='bilinear', align_corners=True)
        ...
        del pre_offset_ref2x, ref2x_feats, pre_offset_ref4x, ref4x_feats, stu_out_ref2x, stu_out_ref4x
        ...
        if paste and self.opt.isTrain:
            head_ref2x = self.ref_head_1(data_ref2x)
            head_ref4x = self.ref_head_2(data_ref4x)
            for i in range(N):
                rand_num = np.random.rand() 
                if rand_num < 0.3:
                    ref2x_deform[i, :, coord_ref2x[i,0]:coord_ref2x[i,1], 
                            coord_ref2x[i,2]:coord_ref2x[i,3]] = head_ref2x[i] 
        ...
        cat_fea_1 = self.concat_fea_1(torch.cat([h, ref2x_deform], 1))
        res_1 = cat_fea_1.clone()

        if self.predict:
            _, _, H, W = data_ref2x_lr.size()
            pre_lr = data_ref2x_lr[:,:, H//4:3*H//4, W//4:3*W//4]
            pre = self.predictor(pre_lr, data_ref4x, cat_fea_1)
        else:
            pre = None

        for i in range(8):
            res_1 = getattr(self, 'block%d'%i)(res_1, pre)

        cat_fea_2 = self.concat_fea_2(torch.cat([res_1, ref4x_deform], 1))
        res_2 = cat_fea_2.clone()
        for i in range(8):
            res_2 = getattr(self, 'block%d'%(i+8))(res_2, pre)

        del res_1, pre, cat_fea_1, cat_fea_2, ref4x_deform, ref2x_deform
        
        res = self.body_lastconv(res_2)
        res += h
        res = self.up(res)
        out = self.tail(res)	
        return out
