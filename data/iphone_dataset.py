...
class IPHONEDataset(BaseDataset):
    def __init__(self, opt, split='train', dataset_name='IPHONE'):
        super(IPHONEDataset, self).__init__(opt, split, dataset_name)
        if self.root == '':
            rootlist = ['']
        ...
        if split == 'train':
            ...
            self.lr_dirs, self.ref2x_dirs, self.hr_dirs, self.names = self._get_image_dir(self.train_root, 'train')
            self._getitem = self._getitem_train
            self.len_data = len(self.names)

        elif split == 'val':
            self.val_root = os.path.join(self.root, 'test')
            self.lr_dirs, self.ref2x_dirs, self.hr_dirs, self.names = self._get_image_dir(self.val_root, 'val') 
            self._getitem = self._getitem_val
            self.len_data = len(self.names)

        elif split == 'test':
            self.test_root = os.path.join(self.root, 'test')
            self.lr_dirs, self.ref2x_dirs, self.hr_dirs, self.names = self._get_image_dir(self.test_root, 'test') 
    ...
    def _get_image_dir(self, dataroot, type):
        lr_dirs = []
        ref2x_dirs = []
        hr_dirs = [] 
        image_names = []

        for video in os.listdir(dataroot + '/LR/'):  
            frames = os.listdir(os.path.join(dataroot, 'LR', video))
            frames = sorted(frames)
            for frame in frames:
                image_names.append(video + '_' + frame)
                lr_dirs.append(os.path.join(dataroot, 'LR', video, frame))
                ref2x_dirs.append(os.path.join(dataroot, 'Ref', video, frame))
                hr_dirs.append(os.path.join(dataroot, 'HR', video, frame))
                if type == 'val':
                    break
